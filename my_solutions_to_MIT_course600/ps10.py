...
class Hand(object):
    ...
    def containsLetters(self, letters):
        ...
        for letter in letters_dict:
            if letter not in self.handDict:
                ...
                if self.handDict[letter] < letters_dict[letter]:
                    return False
        return True
    def isEmpty(self):
        """
    ...
    def __eq__(self, other):
        ...
        for letter in other.handDict:
            if other.handDict[letter] != self.handDict.get(letter, 0):
                return False
        return True
    def __str__(self):
        """
...
class ComputerPlayer(Player):
    ...
    def pickBestWord(self, wordlist):
        ...
        wordlist: sorted by their scores from highest to lowest
        returns: the best word (a string), given the computer player's hand and
        the wordlist; '.' if no words can be made with the given hand.
        """
        best_word = '.'
        for word in wordlist.getList():
            if not self.getHand().isEmpty():
                # print self.getHand().containsLetters(word)
...
class Wordlist(object):
    ...
    def __init__(self):
        ...
        global constant) and stored as a list sorted by their corresponding 
        ...
        for word in self.wordlist:
            word_scores[word] = getWordScore(word)
        sortedWords = sorted(word_scores, key=word_scores.get, reverse=True)
